...
test('Авторизация пользователя с корректными данными (М1-12)', async ({ page }) => {
    ...
    test('Авторизация с пустым паролем показывает клиентскую ошибку (М1-14)', async ({ page }) => {
        const loginPage = new LoginPage(page);
        await loginPage.goto();
        ...
        await test.step('2 Проверка сообщения валидации', async () => {
            await expect(page.getByText('Field cannot be empty.').first()).toBeVisible();
...
test('Авторизация с неверным паролем (М1-25)', async ({ page }) => {
    ...
    test('Запрос восстановления пароля показывает экран отправки письма', async ({ page }) => {
