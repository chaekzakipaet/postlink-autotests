...
test.describe('Модуль 1: Регистрация', () => {
    test('Регистрация нового пользователя (М1-1)', async ({ page }) => {
        ...
        await test.step('2 Заполнение формы и отправка', async () => {
            await registerPage.fillRegistrationForm(testUser.login, testUser.email, testUser.password);
    ...
    test('Регистрация с пустым обязательным полем логина (М1-2)', async ({ page }) => {
        const registerPage = new RegisterPage(page);
    ...
    test('Регистрация с несовпадающими паролями показывает ошибку', async ({ page }) => {
        const registerPage = new RegisterPage(page);
    ...
    test('Регистрация с уже занятым email показывает безопасное сообщение', async ({ page }) => {
        ...
        await registerPage.goto();
        await registerPage.fillRegistrationForm('valid_user', 'taken@example.com', 'ValidPassword123!');
        await acceptRegistrationAgreements(registerPage.modal);
        await registerPage.submit();

        await expect(
            page.getByText("We couldn't complete registration. Check your details, or sign in if you already have an account.").first(),
        ).toBeVisible();
    });
});
