...
export class LoginPage {
  ...
  constructor(page: Page) {
    ...
    this.profileButton = page.locator('button[title="Profile"]');

    this.passwordToggle = this.modal.locator("div").filter({
      has: this.modal.locator("svg"),
    });

    this.errorMessage = page.getByText("Invalid login or password.").first();
    this.forgotPasswordLink = this.modal.getByRole("button", {
